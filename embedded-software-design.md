...
| 项目 | 值 |
|:---:|---|
| 网络拓扑 | PC(USB) → ESP32-S3(CDC转UART) → HC-15B(LoRa) → STM32F411(小车) |
| PC→基站 | USB CDC 虚拟串口（透明串口桥，ESP32-S3 原样转发） |
| 基站→小车 | LoRa 透传（转发 PC 下发的命令文本） |
| GPS 模块 | 可能无信号，演示中不依赖 |

### 17.2 PC 命令集

PC 通过串口助手（115200-8-N-1）发送以下命令，每行以 `\n` 结尾：

| 命令 | 功能 | 说明 |
|:---:|---|---|
| `CHRG\n` | 充电演示 | 小车低速前进 → 检测红外接收管 → ≥3路触发→制动停车（模拟对接） |
| `ALARM\n` | 警演示 | 蜂鸣器 + 红蓝 LED 交替闪烁 60s（或 `STOP\n` 提前停止） |
| `FWD\n` | 前进 | 中速前进 (CCR=5000)，持续到收到 `STOP\n` |
| `BWD\n` | 后退 | 中速后退 (CCR=5000)，持续到收到 `STOP\n` |
| `LEFT\n` | 左转 | 左轮停转，右轮中速转 |
| `RIGHT\n` | 右转 | 右轮停转，左轮中速转 |
| `STOP\n` | 停止 | 所有电机停止，报警停止 |
| `SPD 1|2|3\n` | 设置速度 | 1=低速(33%) / 2=中速(60%) / 3=高速(89%) |

### 17.3 演示代码与正式代码关系

演示模式下：
- **状态机简化为直译模式**：收到命令立即执行对应动作，不经过六状态机
- LoRa 使用了但简化：收到命令帧即执行，不做 ACK 和重传
- 红外对接：模拟完整对接流程（低速前进 → 检测 → 停车上报）
- 报警：完整展示（蜂鸣器+LED+LoRa报警帧）

切换方式：编译时通过 `#define DEMO_MODE 1` 控制。
